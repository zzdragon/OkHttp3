...
public final class Http1ExchangeCodec implements ExchangeCodec {
    ...
    public Source openResponseBodySource(Response response) {
        ...
        if ("chunked".equalsIgnoreCase(response.header("Transfer-Encoding"))) {
            return newChunkedSource(response.request().url(), response.request().body());
        }
    ...
    private Source newChunkedSource(HttpUrl url, RequestBody body) {
        if (state != STATE_OPEN_RESPONSE_BODY) throw new IllegalStateException("state: " + state);
        state = STATE_READING_RESPONSE_BODY;
        return new ChunkedSource(url, body);
    }
    ...
    private class ChunkedSource extends AbstractSource {
        ...
        ChunkedSource(HttpUrl url, RequestBody body) {
            this.url = url;
            this.body = body;
        }
        ...
        private void readChunkSize() throws IOException {
            ...
            if (bytesRemainingInChunk == 0L) {
                ...
                HttpHeaders.receiveHeaders(client.cookieJar(), url, body, trailers);
                responseBodyComplete();
            }
        }
